 ...
 * - get_type: Get type damage relations and Pokémon list
 * - get_ability: Get ability description and Pokémon list
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Pokemon');
}
...
async function getPokemon(name: string) {
  const res = await pwFetch(`${BASE_URL}/pokemon/${encodeURIComponent(name.toLowerCase())}`);
  // A 404 here means "no Pokémon by that name", which is a legitimate ANSWER,
  // not a failure of ours. Throwing booked it as blob4='error' and put this
  // tool on the Problem Tools list at 26.7% while nothing was actually broken.
  // Return a found:false shape so the caller can correct itself and the error
  // metrics keep meaning "we are broken".
  if (res.status === 404) {
    return {
      found: false,
      query: name,
      hint: 'No Pokémon matched that name. PokéAPI expects the lowercase species name (e.g. "pikachu", "mr-mime") or the numeric Pokédex id.',
    };
  }
  if (!res.ok) throw await httpError(res, 'PokéAPI error');
...
async function getType(type: string) {
  const res = await pwFetch(`${BASE_URL}/type/${encodeURIComponent(type.toLowerCase())}`);
  if (!res.ok) throw await httpError(res, 'PokéAPI error');
...
async function getAbility(ability: string) {
  const res = await pwFetch(`${BASE_URL}/ability/${encodeURIComponent(ability.toLowerCase())}`);
  if (!res.ok) throw await httpError(res, 'PokéAPI error');
...
async function getEvolutionChain(id: number) {
  const res = await pwFetch(`${BASE_URL}/evolution-chain/${id}`);
  if (!res.ok) throw await httpError(res, 'PokéAPI error');
